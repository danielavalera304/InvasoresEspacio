...
//DECLARO LAS CONSTANTES
    ...
    //para ponerle la velocidad= 10 milisegundos
    ...
    //contador para saber el numero de veces que se ha ejecutado el bucle de animación
    int contador = 0;
    ...
    public VentanaJuego() {
        ...
        for(int i=0; i<5; i++){ //creo 5 columnas. Esto es la Y
            for (int j=0; j<10; j++){ //con 10 marcianos. Esto es la X
                Marciano miMarciano = new Marciano();
                miMarciano.setX(j* (15 + miMarciano.imagen.getWidth(null)));
                miMarciano.setY(i* (8 + miMarciano.imagen.getHeight(null)));
       ...
       miNave.setY(ALTOPANTALLA - ALTOPANTALLA/6);
       //es para que se inicie
       temporizador.start();
    }
    ...
    for (int i = 0; i < listaDisparos.size(); i++) {
            ...
            //recoloco el disparo
            aux.mueve();
            //si la posición Y del disparo es menor o igual a 0, es que
    ...
    private void chequeaColision(){
        ...
        for (int j=0; j<listaDisparos.size(); j++){
            ...
            for (int i = 0; i < listaMarcianos.size(); i++){
                ...
                //asigno  el arco a la posicion del marciano
                rectanguloMarciano.setFrame(m.getX(), m.getY(), m.imagen.getWidth(null), m.imagen.getHeight(null));
                if (rectanguloDisparo.intersects(rectanguloMarciano)){
                    //creo una explosion y la añado en la posicion en la que está el marciano
    ...
    //este evento está en el jFrame porque si lo pongo en el jPanel tendría que esperar a que hiciera clik, en cambio
    // en el jFrame se inicializa nada más abrirse
    //pressed= cuando presionas la tecla
...
//                }
    ...
    //released= cuando levantas la tecla
